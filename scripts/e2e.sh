...
#
#   sh scripts/e2e.sh                                   # deploys build/app.wasm, sends the template's transfer
...
if [ -z "${PAYLOAD:-}" ]; then
  # the template's own instruction: a transfer to the signing address itself
  PAYLOAD="{\"type\":\"transfer\",\"to\":\"$ME\",\"amount\":\"1000\"}"
fi
echo "== send $PAYLOAD"; run send "$PAYLOAD"
if [ -z "${PAYLOAD2:-}" ] && [ "$PAYLOAD" = "{\"type\":\"transfer\",\"to\":\"$ME\",\"amount\":\"1000\"}" ]; then
...
echo "== app-events (public)"; run app-events 2
